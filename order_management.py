...
def add_customer(main_window):
    ...
    def preview_orders():
        ...
        for order in order_list.get(0, END):
            Label(preview_window,
                  text=f"BF: {order[0]}, Size: {order[1]}, GSM: {order[2]}, Type: {order[3]}, Qty: {order[4]}").pack()
    ...
    Button(add_customer_window, text="Add Order", command=add_order).grid(row=6, column=0, columnspan=3)

    order_list = Listbox(add_customer_window, width=60)
    order_list.grid(row=7, column=0, columnspan=3)

    Button(add_customer_window, text="Preview Orders", command=preview_orders).grid(row=8, column=0, columnspan=3)
    Button(add_customer_window, text="Save", command=save_customer).grid(row=9, column=0, columnspan=3)
...
def view_all_orders(main_window):
    ...
    for order in orders:
        ...
        pc.execute("SELECT COUNT(*) FROM products WHERE product_type=? AND size=? AND gsm=?", (type_, size, gsm))
